#!/usr/bin/env python3
"""Evaluate the frozen M7 JPEG-consistency pilot on WildDeepfake."""
...
from evaluate_xception_jpeg_consistency import (
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=(
            project_root
            / "configs/wilddeepfake_evaluation_v1/protocol.json"
        ),
    )
    parser.add_argument(
        "--pilot-config",
...
def build_report(summary: pd.DataFrame) -> str:
    ...
    return "\n".join(
        [
            "# M7 JPEG-Consistency Pilot on WildDeepfake",
            "",
            (
            ...
            "",
            "| Model | Seed | Sequence AUC | Sequence AUPRC | Real FPR | Fake recall | Frame AUC |",
            "|---|---:|---:|---:|---:|---:|---:|",
            (
                f"| M7 consistency | 42 | {row.sequence_roc_auc:.4f} | "
            ...
            "",
            (
                "Compare this row with the existing M7 Mixed-JPEG seed-42 "
                "result produced by the same native RGB/Letterbox-299 test "
                "path. Do not change the consistency weight or training "
                "transform in response to WildDeepfake results."
            ),
            "",
        ]
    )
...
def main() -> None:
    ...
    checkpoint_info = validate_pilot_checkpoint(
        checkpoint,
        checkpoint_path,
