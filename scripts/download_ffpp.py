...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default='raw',
        help='Which compression degree. Raw (c0) videos are lossless compressed.',
        choices=COMPRESSION
    )
    parser.add_argument(
        '-t', '--type',
        type=str,
        default='videos',
        help='Which file type, i.e. videos, masks, or models.',
        choices=TYPE
    )
    parser.add_argument(
        '-n', '--num_videos',
        type=int,
        default=None,
        help="Select a number of videos if you don't want to download the full dataset."
    )
    parser.add_argument(
        '--server',
        type=str,
    ...
    if server == 'EU':
        ...
        raise Exception(f'Wrong server name. Choices: {SERVERS}')
    ...
    for dataset in c_datasets:
        ...
        if 'DeepFakeDetection' in dataset_path or 'actors' in dataset_path:
            ...
            file_pairs = json.loads(
                urllib.request.urlopen(args.base_url + '/' + FILELIST_URL).read().decode("utf-8")
            )
            filelist = []
